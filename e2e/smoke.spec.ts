...
test.describe('Resource Explorer - Smoke Tests', () => {
  ...
  test('should load homepage and display Pokemon list', async ({ page }) => {
    ...
    await expect(
      page.locator('h1:has-text("Resource Explorer")')
    ).toBeVisible();
  ...
  test('should search for Pokemon', async ({ page }) => {
    ...
    await page.waitForFunction(
      () => {
        ...
        return (
          heading && heading.textContent?.toLowerCase().includes('pikachu')
        );
      },
      { timeout: 10000 }
    );
  ...
  test('should navigate to detail page', async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("Add to favorites")')
    ).toBeVisible();
  });
  ...
  test('should filter by type', async ({ page }) => {
    ...
    const typeLabels = page.locator(
      '[data-testid="pokemon-type"]:has-text("fire")'
    );
  ...
  test('should handle errors gracefully', async ({ page }) => {
    ...
    await page.waitForSelector('text=Failed to load Pokémon', {
      timeout: 10000,
    });
